...
CDescriptorSetLayout::Builder &CDescriptorSetLayout::Builder::AddBinding(uint32_t a_binding, VkDescriptorType a_descriptorType, VkShaderStageFlags a_stageFlags, uint32_t a_count)
{
...
CDescriptorSetLayout::CDescriptorSetLayout( const std::shared_ptr<CDevice>& a_pDevice, std::unordered_map<uint32_t, VkDescriptorSetLayoutBinding> a_bindings)
...
CDescriptorPool::CDescriptorPool(const std::shared_ptr<CDevice>& a_pDevice, uint32_t a_maxSets, VkDescriptorPoolCreateFlags a_poolFlags, const std::vector<VkDescriptorPoolSize> &a_poolSizes)
    : m_pDevice(a_pDevice)
{
...
bool CDescriptorWriter::Build(VkDescriptorSet &a_set) {
  bool success = m_pool.AllocateDescriptor(m_setLayout.GetDescriptorSetLayout(), a_set);
  if (!success) {
    return false;
  }
  Overwrite(a_set);
